...
class _AdminDashboardScreenState extends State<AdminDashboardScreen> {
  ...
  Future<void> _deletePost(String id) async {
    final confirm = await showDialog<bool>(
      ...
      builder: (_) => AlertDialog(
        ...
        content: const Text('This will permanently delete this announcement. Are you sure?'),
        shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(14)),
        actions: [
          TextButton(onPressed: () => Navigator.pop(context, false), child: const Text('Cancel')),
          TextButton(
            onPressed: () => Navigator.pop(context, true),
            child: const Text('Delete', style: TextStyle(color: AppColors.urgent)),
          ),
        ],
      ),
    );
    ...
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        title: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          ...
          if (_user != null)
            Text('${_user!.fullName} · ${_user!.department}',
  ...
  Widget _buildDashboardView() {
    return ListView(
      ...
      children: [
        // Stat cards
        Row(children: [
          _statCard(_totalPosts.toString(), 'Posts this week', AppColors.primary),
          const SizedBox(width: 8),
          _statCard(_studentsReached > 0 ? '${(_studentsReached / 1000).toStringAsFixed(1)}k' : '0', 'Students reached', AppColors.info),
          const SizedBox(width: 8),
        ...
        // Recent posts
        Row(children: [
          const Text(AppStrings.myRecentPosts,
              style: TextStyle(fontSize: 14, fontWeight: FontWeight.w500, color: AppColors.textPrimary)),
          const Spacer(),
          GestureDetector(
            onTap: () => setState(() => _currentNavIndex = 2),
            child: const Text('Analytics', style: TextStyle(fontSize: 12, color: AppColors.primary)),
          ),
        ]),
        const SizedBox(height: 8),
  ...
  Widget _emptyPosts() => Container(
    ...
    child: const Center(child: Column(children: [
      Icon(Icons.campaign_outlined, size: 40, color: AppColors.textSecondary),
      SizedBox(height: 10),
      Text('No posts yet', style: TextStyle(fontSize: 13, fontWeight: FontWeight.w500, color: AppColors.textPrimary)),
      SizedBox(height: 4),
      Text('Tap the button above to post your first announcement', textAlign: TextAlign.center,
    ...
    return Container(
      ...
      child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        Row(children: [
          ...
          const Spacer(),
          Text(post.timeAgo, style: const TextStyle(fontSize: 10, color: AppColors.textSecondary)),
        ]),
    ...
    return ListView(
      ...
      children: [
        ...
        Row(children: [
          ...
          const SizedBox(width: 8),
          _statCard('${_studentsReached > 0 ? (_studentsReached / 1000).toStringAsFixed(1) : 0}k', 'Total reached', AppColors.primary),
        ]),
        const SizedBox(height: 16),
        ...
        Container(
          ...
          decoration: BoxDecoration(color: AppColors.white, borderRadius: BorderRadius.circular(12), border: Border.all(color: AppColors.border, width: 0.5)),
          child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
            const Text('Read rate by category',
        ...
        // Top posts
        const Text('Recent posts performance',
            style: TextStyle(fontSize: 13, fontWeight: FontWeight.w500, color: AppColors.textPrimary)),
        const SizedBox(height: 8),
        if (_myPosts.isEmpty)
          _emptyPosts()
  ...
  Widget _rateBar(String label, double rate, Color color) {
    return Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
      Row(children: [
        SizedBox(width: 100, child: Text(label, style: const TextStyle(fontSize: 11, color: AppColors.textSecondary))),
  ...
  Widget _analyticsCard(AnnouncementModel post) {
    return Container(
      ...
      child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        Row(children: [_categoryChip(post.category), const Spacer(), Text(post.timeAgo, style: const TextStyle(fontSize: 10, color: AppColors.textSecondary))]),
        const SizedBox(height: 6),
        Text(post.title, style: const TextStyle(fontSize: 13, fontWeight: FontWeight.w500, color: AppColors.textPrimary), maxLines: 2, overflow: TextOverflow.ellipsis),
        const SizedBox(height: 6),
        Row(children: [
          const Icon(Icons.people_outline, size: 12, color: AppColors.textSecondary),
          const SizedBox(width: 4),
          Text('247 reached · ', style: const TextStyle(fontSize: 11, color: AppColors.textSecondary)),
          Text('91% read', style: const TextStyle(fontSize: 11, fontWeight: FontWeight.w500, color: AppColors.primary)),
          Text(' · within 8 min', style: const TextStyle(fontSize: 11, color: AppColors.textSecondary)),
        ]),
      ]),
    );
  }
  ...
  Widget _statCard(String num, String label, Color color) => Expanded(
    child: Container(
      ...
      child: Column(children: [
        Text(num, style: TextStyle(fontSize: 20, fontWeight: FontWeight.w500, color: color)),
  ...
  Widget _categoryChip(String category) {
    Color bg; Color text;
    switch (category) {
      case 'urgent': bg = AppColors.urgentLight; text = AppColors.urgent; break;
      case 'academic': bg = const Color(0xFFEEEDFE); text = const Color(0xFF3C3489); break;
      case 'exam': bg = AppColors.warningLight; text = AppColors.warning; break;
      default: bg = AppColors.infoLight; text = AppColors.info;
    }
    return Container(
    ...
    return Container(
      ...
      child: BottomNavigationBar(
        ...
        items: const [
          ...
          BottomNavigationBarItem(icon: Icon(Icons.campaign_outlined), activeIcon: Icon(Icons.campaign), label: 'Post'),
